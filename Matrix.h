...
void* f_Mult_thread(void* ar){
	...
	for (unsigned int l=0;l<arg->ptr[1]->w;l++) {
		f = f_Matrix_get(arg->ptr[1],arg->l,arg->k)*f_Matrix_get(arg->ptr[2],arg->m,l);
		f_Matrix_set(arg->ptr[0],arg->m,arg->k,f+f_Matrix_get(arg->ptr[0],arg->m,arg->k));
	}
	arg->finished[arg->m+arg->k*arg->ptr[0]->w]=1;
	pthread_exit(NULL);
}
...
int f_Matrix_multiply(f_Matrix_t *m1,f_Matrix_t *m2,f_Matrix_t* out){
	if ((m1->w==m2->h)&(out->h==m1->h)&(out->w==m2->w)){
		...
		char * finnished =(char*) calloc(m1->h*m2->w,sizeof(char));
		float f = 0;
		...
		unsigned int finished_num;
		while(finished_num<m1->h*m2->w){
		finished_num=0;
		for (unsigned int k=0; k<m1->h;k++) {
			for (unsigned int m=0; m<m2->w; m++) {
				finished_num+=finnished[m+k*m2->w];
			}
		}
		}
		for (unsigned int k=0; k<m1->h;k++) {
			for (unsigned int m=0; m<m2->w; m++) {
				free(args[m+k*m2->w].ptr);
			}
		}
		free(args);
		free(finnished);
